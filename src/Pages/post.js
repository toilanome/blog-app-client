...
const Post = ({ posts }) => {
  ...
  const filterPosts = selectCategoryId ? thePost?.filter((post) => post?.category?._id === selectCategoryId) : thePost
            ...
            <>
              ...
              <div>
                ...
                <Link to={`/post/${item._id}`} >
                  <img src={`http://localhost:4000/${item.cover}`  }  onClick={scrollTop}/>
                </Link>
                </div>
                ...
                <div className="bg-cover flex flex-col justify-center items-center">
                  <div className="ml-[82px] mr-[82px] w-[471px] h-[185px]">
                    ...
                    <h5 className="font-bold text-lg cursor-pointer" onClick={scrollTop}>{item?.title}</h5>
                 </Link>
                    ...
                    <div className="mt-8 flex items-center justify-between ">
                      ...
                      <div className="flex items-center ">
                        <Link to={`/post/${item._id}`}>
                          <span className="mr-2 font-extrabold " onClick={scrollTop}>
                            Read Full
                          </span>
                        </Link>
                        <FaArrowRight />
        ...
        <div>
          <div className="grid grid-cols-3 gap-28">
            {thePost?.slice(1,4).map((item) =>(
              ...
              <div className="mb-5">
              <Link to={`/post/${item._id}`}>
                  <img src={`http://localhost:4000/${item.cover}`  } className="rounded-md h-52"  onClick={scrollTop}/>
                </Link>
              </div>
              <div className="h-64">
                <div>
                  <span className="mr-5 text-xs">{item?.author?.userName}</span>
                  <span className="text-xs text-[#6E6E6E] ">{formatISO9075(new Date(item.createdAt))} </span>
                </div>

                <Link to={`/post/${item._id}`}>
                <h5 className="font-extrabold text-xl break-words hover:text-gray-700" onClick={scrollTop}>
                  {item.title}
                </h5>
                ...
                <div className="mt-8 flex items-center justify-between ">
                  ...
                  <div className="flex items-center ">
                  <Link to={`/post/${item._id}`}>
                          <span className="mr-2 font-extrabold hover:text-gray-700 " onClick={scrollTop}>
                            Read Full
                          </span>
                        </Link>
                    <FaArrowRight />
      ...
      <section className="mt-20 mb-20 container">
        ...
        <div>
          <div className="grid grid-cols-3  gap-12">
              ...
              <div className="mr-5">
              <Link to={`/post/${item._id}`}>
                  <img src={`http://localhost:4000/${item.cover}`  } className="h-24 w-full hover:text-gray-700" onClick={scrollTop}/>
                </Link>
              </div>
              <div className=" flex flex-col  items-center">
                <div className="space-y-1">
                <Link to={`/post/${item._id}`}>
                <h5 className="font-bold text-lg break-words hover:text-gray-700" onClick={scrollTop}>
                    {item.title}
                  </h5>
                </Link>
            ...
            <div className="p-6 border border-gray-300 rounded-lg">
              <div className="grid grid-cols-2 gap-7">
                {filterPosts?.slice(3,4).map((item) =>(
                  ...
                  <div className="mb-6">
                    <Link to={`/post/${item._id}`}>
                    <img src={`http://localhost:4000/${item.cover}`} className="rounded-lg" onClick={scrollTop}/>
                  ...
                  <div>
                    ...
                    <h5 className="font-extrabold text-xl break-words mb-5" onClick={scrollTop}>
                      {item.title}
                    </h5>
                    </Link>
                    ...
                    <div className="mt-12 flex items-center ">
                      <Link className="mr-2 font-extrabold text-sm " to={`/post/${item._id}`} onClick={scrollTop}>
                        Load More
                      </Link>
                      <FaArrowRight />
                    </div>
                ...
                {filterPosts?.slice(3,7 ).map((item) =>(
                  ...
                  <div className="flex gap-3 mb-9" key={item._id}>
                    ...
                    <Link to={`/post/${item._id}`} >
                      ...
                      <img src={`http://localhost:4000/${item.cover}`} className="w-40 rounded-lg"  onClick={scrollTop}/>
                      </Link>
                    </div>
                    <div>
                    <Link to={`/post/${item._id}`}>

                      <h5 className="font-bold text-lg break-words" onClick={scrollTop}>
                        {item.title}
                      </h5>
                      </Link>
            ...
            <div className="border border-gray-300 rounded-lg p-4 pl-10">
              ...
              {thePost?.slice(2,7).map((item) =>(
                ...
                <Link to={`/post/${item._id}`}>
                  ...
                  <h5 className="font-bold text-sm break-words" onClick={scrollTop}>
                    {item.title}
                  </h5>
                  </Link>
